...
public class KieHelper {
	...
	public static KieContainer getKieContainer(ReleaseId releaseId, ArrayList<Resource> resources,
			Class testClass) {
        KieServices ks = KieServices.Factory.get();
        createAndDeployJar( ks, releaseId, resources );
        return ks.newKieContainer( releaseId );
    ...
    public static byte[] createJar(KieServices ks, ReleaseId releaseId, ArrayList<Resource> resourceNames) {
        KieFileSystem kfs = ks.newKieFileSystem().generateAndWritePomXML( releaseId );
